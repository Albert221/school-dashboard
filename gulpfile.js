...
const otherFiles = [
    'src/**/*',
    '!src/scss' // Do not create empty directry for scss
].concat([htmlFiles, scssFiles, jsFiles].map((pattern) => `!${pattern}`));
...
gulp.task('bs', () => {
    ...
    gulp.watch([htmlFiles, scssFiles, jsFiles], ['html']);
    gulp.watch(otherFiles, ['copy']).on('change', browserSync.reload);
});
